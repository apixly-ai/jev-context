...
看板输出带访问令牌的 `127.0.0.1` 地址，按 Ctrl-C 停止服务。不指定 `--port` 时先尝试 8765，被占用则自动选择空闲端口。显式指定的端口冲突返回 `PortInUse`；`--port 0` 总是选择空闲端口。可手动刷新、按模型、UTC 日期和计数方式筛选，以及导出当前筛选的全部 JSON。卡片、图表和明细采用同一筛选范围；表格显示最近 100 条。无需前端构建、外部脚本或云服务。
