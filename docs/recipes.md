...
```json
[
  {"id":"req-17","text":"TLS verification failed before any HTTP response.","source":{"request_id":"req-17"}},
  {"id":"req-18","text":"An earlier timeout recovered. Current request returned HTTP 200.","source":{"request_id":"req-18"}}
]
```
...
```sh
jev-context exec --analysis analysis.json \
...
```sh
jev-context query --input examples/candidates.json \
...
```sh
jev-context code-search 'retry|backoff' --root ./src \
...
```sh
jev-context locate --session YOUR_SESSION --tab YOUR_TAB \
...
```sh
jev-context triage --input examples/events.jsonl \
