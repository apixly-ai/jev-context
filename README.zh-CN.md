...
配置本地数值账本后，运行 `jev-filter stats dashboard`（8765 被占用时自动选择空闲端口）即可查看估算输入 token 减少量、美元输入价值、Jev 成本与净值，按模型/日期筛选，并导出 JSON 或独立 HTML。未知和负收益明确保留；这些是输入等价值估算，不是账单节省。参见[配置与统计口径](docs/statistics.zh-CN.md)。
